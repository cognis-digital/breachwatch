...
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'BW-HIT-001', 'severity': 'high', 'weight': 2.5, 'title': 'EMAIL_IN_BREACH', 'when': {'in_breach': True}, 'remediation': 'Rotate password; enable MFA; check reuse.'}, {'id': 'BW-STEALER-001', 'severity': 'critical', 'weight': 3.0, 'title': 'STEALER_LOG_MATCH', 'when': {'in_stealer_log': True}, 'remediation': 'Assume full device compromise. Rebuild + rotate.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="breach",
                    metadata=item,
                ))
