"""Native cognis-connect emit for breachwatch — forward findings to any platform.

Maps breachwatch's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    breachwatch ... --format json | breachwatch-emit --to stix
    breachwatch-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('breach', '') + ' ' + rec.get('source', '')
