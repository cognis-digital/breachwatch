...
**breachwatch** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `breachwatch` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
breachwatch ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
breachwatch ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
breachwatch ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
```
...
works whatever `breachwatch` outputs:

```python
from cognis_connect import normalize, misp
findings = [normalize(rec, source="breachwatch") for rec in records]   # records = your JSON output
